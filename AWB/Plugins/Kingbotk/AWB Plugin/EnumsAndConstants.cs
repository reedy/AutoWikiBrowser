...
namespace AutoWikiBrowser.Plugins.Kingbotk
{
	...
	static internal class Constants
	{
		...
		internal const string conRegexpRight = ")\\b\\s*(((\\|\\|*| \\||\\| |\\s*\\|\\s*(?<parm>[^}{|\\s=]*))\\s*)+(=\\s*(?<val>[^}{|\\n\\r]*?)\\s*)?)*\\}\\}\\s*";

			//")\b[^}]*"
		internal const string conRegexpRightNotStrict = ")\\b";

		internal const RegexOptions conRegexpOptions = RegexOptions.Compiled | RegexOptions.Multiline | RegexOptions.IgnoreCase | RegexOptions.ExplicitCapture;
		// Identifiers:
