//$Header: /cvsroot/autowikibrowser/WikiFunctions/ReplaceSpecial/Rule.cs,v 1.2 2006/07/04 15:50:24 wikibluemoose Exp $
/*
...
internal class Rule: IRule
{
  ...
  public override void WriteToXml(TreeNode tn, XmlTextWriter w)
  {
    ...
    w.WriteStartElement(Rule.XmlName);
    
    w.WriteAttributeString("name", r.Name);
    w.WriteAttributeString("type", ((int)r.ruletype_).ToString());
    w.WriteAttributeString("enabled", r.enabled_.ToString());
 
    if (r.numoftimes_ != 1)
      w.WriteAttributeString("times", r.numoftimes_.ToString());
    
    if (r.regex_)
      RegexXml.Write(w, r.regexOptions_);
    
    if (r.replace_ != "") {  
      w.WriteStartElement("replace");
      w.WriteString(r.replace_);
      w.WriteEndElement();
    }
    
    if (r.with_ != "") {
      w.WriteStartElement("with");
      w.WriteString(r.with_);
      w.WriteEndElement();
    }
  ...
  void WriteIfToXml(XmlTextWriter w)
  {
    if (this.ifContains_ == "" && this.ifNotContains_ == "")
      return;

    w.WriteStartElement("if");
    
    if (this.ifIsRegex_)
      RegexXml.Write(w, this.ifRegexOptions_);

    if (this.ifContains_ != "") {
      w.WriteStartElement("contains");
      w.WriteString(this.ifContains_);
      w.WriteEndElement();
    }
    if (this.ifNotContains_ != "") {
      w.WriteStartElement("notcontains");
      w.WriteString(this.ifNotContains_);
      w.WriteEndElement();
    }
