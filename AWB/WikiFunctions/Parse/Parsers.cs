...
namespace WikiFunctions.Parse
{
    ...
    public class Parsers
    {
        ...
        private static readonly Regex FootnoteAfterDupePunctuation = new Regex(@"([^,\.:;])" + RefsPunctuation + @"\2 *(?<sfn>" + Footnote.ToString() + @"|" + InlineTemplate.ToString() + @")");
        ...
        public static string RefsAfterPunctuation(string articleText)
        {
            ...
            bool HasInlineTemplate = InlineTemplate.IsMatch(articleText);
            ...
            if(HasFootnote || HasInlineTemplate)
            {
        ...
        private static readonly Regex citeWebParameters = new Regex(@"\b(accessdate|agency|archivedate|archiveurl|article|arxiv|asin|asin-tld|at|author\d?|authorlink\d?|author\d*-?link|authors?|author-mask|author-name-separator|author-separator|bibcode|chapter|chapter\-?url|coauthors?|contribution|contribution\-?url|date|deadurl|dictionary|display\-?authors|display\-?editors|distributor|doi|doibroken|doi_brokendate|edition|editors?|editor\d*|editor\d*-first|editor\d*-last|editor\d*-link|embargo|encyclopa?edia|entry|first\d*|format|id|institution|isbn|ISBN|issue|issn|jfm|journal|jstor|language|last\d*|lastauthoramp|laydate|laysource|laysummary|layrul|lccn|location|magazine|month|mr|newspaper|nopp|oclc|ol|origyear|others|osti|pages?|periodical|place|pmc|pmid|postscript|publication\-?(?:place|date)|publisher|quote|ref|registration|rfc|section|sectionurl|separator|series|subscription|ssrn|title|trans_title|type|url|version|via|volume|website|work|year|zbl)\b", RegexOptions.Compiled);
