...
namespace AutoWikiBrowser.Plugins.Kingbotk
{
	...
	internal abstract class PluginBase
	{
		...
		protected static readonly Regex PreferredTemplateNameRegexCreator = new Regex("^(?<first>[a-zA-Z0-9]{1})(?<second>.*)", RegexOptions.Compiled);
		protected string PreferredTemplateNameWikiMatchEvaluator(Match match)
		{
			return "^[" + match.Groups["first"].Value.ToUpper() + match.Groups["first"].Value.ToLower() + "]" + match.Groups["second"].Value + "$";
		}
		...
		{
			return ProcessTalkPage(A, Classification.Code, Importance.Code, false, false, false, ProcessTalkPageMode.Normal, AddReqPhotoParm);
		}
		...
		{
			Template.NewOrReplaceTemplateParm(ParamName, "yes", article, true, true, ParamAlternativeName: ParamAlternativeName, PluginName: PluginShortName);
		}
		...
		protected void StubClass()
		{
			if (article.Namespace == Namespace.Talk) {
				if (GenericSettings.StubClass) {
					Template.NewOrReplaceTemplateParm("class", "Stub", article, true, false, PluginName: PluginShortName, DontChangeIfSet: true);
				}

				if (GenericSettings.AutoStub && Template.NewOrReplaceTemplateParm("class", "Stub", article, true, false, PluginName: PluginShortName, DontChangeIfSet: true))
					AddAndLogNewParamWithAYesValue("auto");
				// If add class=Stub (we don't change if set) add auto
			}
		}

		protected void ReplaceATemplateWithAYesParameter(Regex R, string ParamName, string TemplateCall, bool Replace = true)
		{
			var _with5 = article;
