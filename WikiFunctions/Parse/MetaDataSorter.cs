...
<!--InterwikiLocalFirstEnds-->";
        ...
        private const string ProdBlpComment = @"<!-- Do not use the ""prod blp/dated"" template directly; the above line is generated by ""subst:prod blp|reason"" -->";
		...
		internal string Sort(string articleText, string articleTitle, bool fixOptionalWhitespace)
		{
            ...
            if (TemplateExists(alltemplates, WikiRegexes.DeletionProtectionTags))
            {
                ...
                // move prod blp comment as well so it's kept with template
                if(articleText.Contains(ProdBlpComment) && Tools.NestedTemplateRegex("Prod blp/dated").IsMatch(articleText))
                {
                    articleText = articleText.Replace(ProdBlpComment, "");
                    Match prodblp = Tools.NestedTemplateRegex("Prod blp/dated").Match(articleText);
                    articleText = articleText.Insert(prodblp.Index+prodblp.Length, " " + ProdBlpComment);
                }
            }
