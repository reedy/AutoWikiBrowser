...
| cn1=21st century
            ...
            ClassicAssert.IsTrue(WikiRegexes.Orphan.IsMatch(text), "Tag for orphan");
        ...
        public void RenameUncategorised()
        {
            ...
            ClassicAssert.IsFalse(Tools.NestedTemplateRegex("Uncategorized stub").IsMatch(text), "uncategorized not renamed when not stub");
            ClassicAssert.IsFalse(WikiRegexes.Stub.IsMatch(text));

            text = parser.Tagger(LongText + @"{{stub}} {{Uncategorised|date=May 2010}}", "Test", false, out noChange, ref summary);
            ClassicAssert.IsTrue(Tools.NestedTemplateRegex("Uncategorised").IsMatch(text), "Uncategorised not renamed when stub removed");
            ClassicAssert.IsFalse(WikiRegexes.Stub.IsMatch(text));
            ...
            ClassicAssert.IsTrue(Tools.NestedTemplateRegex("Uncategorized stub").IsMatch(text), "Uncat stub retained");
            ClassicAssert.IsFalse(Tools.NestedTemplateRegex("Uncategorised").IsMatch(text), "Uncategorised removed when already uncat stub");
            ClassicAssert.IsTrue(WikiRegexes.Stub.IsMatch(text));
            ...
            ClassicAssert.IsTrue(WikiRegexes.Wikify.IsMatch(text), "wikify tag with reason NOT removed");
...
{{Uncategorised|date=May 2010}}{{stub}}", "Test", false, out noChange, ref summary);
        ...
        public void RenameUnreferenced()
        {
            ...
            ClassicAssert.IsTrue(text.Contains("unreferenced"), "Unref when no refs");

            text = parser.Tagger(ShortText + @"{{unreferenced|date=May 2010}} <!--<ref>foo</ref>-->", "Test", false, out noChange, ref summary);
            ClassicAssert.IsTrue(text.Contains("unreferenced"), "Unref when no refs 2");

            text = parser.Tagger(ShortText + @"{{unreferenced|date=May 2010}} <ref>foo</ref>", "Test", false, out noChange, ref summary);
            ClassicAssert.IsFalse(WikiRegexes.Unreferenced.IsMatch(text), "Unref to more citations needed when no refs 3");
            ClassicAssert.IsTrue(text.Contains("more citations needed"), "Unref when no refs 4");
            ...
            ClassicAssert.IsFalse(WikiRegexes.Unreferenced.IsMatch(text), "Unref to refimprove when has refs");
            ClassicAssert.IsTrue(text.Contains("more citations needed"), "Renames unreferenced to more citations needed in MI when existing refs");

            text = parser.Tagger(ShortText + @"{{unreferenced|date=May 2010}} <ref group=X>foo</ref>", "Test", false, out noChange, ref summary);
            ClassicAssert.IsTrue(WikiRegexes.Unreferenced.IsMatch(text), "Unref remains when only grouped footnote refs");
            ...
            ClassicAssert.IsFalse(WikiRegexes.Unreferenced.IsMatch(text), "Unref to refimprove when has sfn refs");
            ClassicAssert.IsTrue(WikiRegexes.MultipleIssues.Match(text).Value.Contains("more citations needed"), "Renames unreferenced to more citations needed in MIwhen existing refs");
            ...
            ClassicAssert.IsFalse(WikiRegexes.Unreferenced.IsMatch(text), "Unref to more citations needed when has efn refs");
            ClassicAssert.IsTrue(WikiRegexes.MultipleIssues.Match(text).Value.Contains("more citations needed"), "Renames unreferenced to more citations needed in MI when existing refs");
            ...
            ClassicAssert.IsFalse(WikiRegexes.Unreferenced.IsMatch(text), "Unref removed when more citations needed");
...
{{unreferenced|section=yes|date=May 2010}} {{more citations needed|date=May 2010}} <ref>foo</ref>", "Test", false, out noChange, ref summary);
            ClassicAssert.IsTrue(WikiRegexes.Unreferenced.IsMatch(text), "Unref retained when section template");
...
{{more citations needed|date=May 2010}}
            ...
            ClassicAssert.IsFalse(WikiRegexes.Unreferenced.IsMatch(text), "Unref removed when refimprove");
            Assert.That(Tools.NestedTemplateRegex("more citations needed").Matches(text).Count, Is.EqualTo(1));
        ...
        public void AddDeadEnd()
        {
            ...
            ClassicAssert.IsFalse(text.Contains("Underlinked"), "underlinked removed when dead end");
            ClassicAssert.IsTrue(WikiRegexes.DeadEnd.IsMatch(text), "page is deadend");
            ClassicAssert.IsTrue(summary.Contains("removed underlinked"));

            text = parser.Tagger(ShortText + @"{{shipindex}}", "Test", false, out noChange, ref summary);
            ClassicAssert.IsFalse(WikiRegexes.DeadEnd.IsMatch(text), "SIA page not tagged as deadend");
        }
        ...
        public void AddMultipleTagsWihtespace()
        {
            ...
            string text = parser.Tagger(ShortText, "Test", false, out noChange, ref summary);
            ClassicAssert.IsTrue(text.Contains(@"{{Dead end|date={{subst:CURRENTMONTHNAME}} {{subst:CURRENTYEAR}}}}
{{Orphan|date={{subst:CURRENTMONTHNAME}} {{subst:CURRENTYEAR}}}}"), "No excess blank line between multiple tags");
        ...
        public void Add()
        {
            ...
            ClassicAssert.IsTrue(ShortText == text, "nothing changed");
...
{{DEFAULTSORT:bJqnzFm7e, opFhLKq7z}}";
            ...
            ClassicAssert.IsTrue(WikiRegexes.Wikify.IsMatch(text));
            ClassicAssert.IsTrue(text.StartsWith(@"{{Underlinked|date={{subst:CURRENTMONTHNAME}} {{subst:CURRENTYEAR}}}}
...
{{temp}}"), "no excess newlines on tag change");
            ...
            ClassicAssert.IsFalse(text.Contains(@"{{Underlinked|date={{subst:CURRENTMONTHNAME}} {{subst:CURRENTYEAR}}}}"), "SIA pages not tagged as underlinked");

            text = parser.Tagger(@"{{disambiguation}} [[a]]" + LongText, "Test", false, out noChange, ref summary);
            ClassicAssert.IsFalse(text.Contains(@"{{Underlinked|date={{subst:CURRENTMONTHNAME}} {{subst:CURRENTYEAR}}}}"), "disambig pages with 1 link not tagged as underlinked");

            Variables.SetProjectLangCode("en");
            WikiRegexes.MakeLangSpecificRegexes();
            text = parser.Tagger(t1 + LongText, "Test", false, out noChange, ref summary);
            ClassicAssert.IsFalse(text.StartsWith(@"{{Underlinked|date={{subst:CURRENTMONTHNAME}} {{subst:CURRENTYEAR}}}}
{{temp}}"), "Don't add underlinked for en-wiki");
        ...
        public void AddSv()
        {
            ...
            ClassicAssert.IsFalse(text.Contains("{{Wikify|" + WikiRegexes.DateYearMonthParameter + @"}}

"));
            ClassicAssert.IsTrue(Tools.NestedTemplateRegex("Okategoriserad").IsMatch(text));
            ClassicAssert.IsTrue(WikiRegexes.Stub.IsMatch(text));
            Variables.SetProjectLangCode("en");
            WikiRegexes.MakeLangSpecificRegexes();
#endif
        ...
        public void AddDeadEndAr()
        {
            ...
            ClassicAssert.IsTrue(WikiRegexes.DeadEnd.IsMatch(text), "page is deadend");
            ClassicAssert.IsFalse(WikiRegexes.Wikify.IsMatch(text), "wikify should not be present which dead end added");
            ClassicAssert.IsFalse(text.Contains("وصلات قليلة"), "wikify removed when dead end");
            ClassicAssert.IsTrue(summary.Contains("وصلات قليلة"));
        ...
        public void AddAr()
        {
            ...
            ClassicAssert.IsFalse(text.Contains("Uncategorized"), "no en-wiki uncat tags");
            ClassicAssert.IsTrue(WikiRegexes.Stub.IsMatch(text), " stub");
            ...
            ClassicAssert.IsFalse(text.Contains("{{يتيمة|" + WikiRegexes.DateYearMonthParameter + @"}}"), "orphan");
            ClassicAssert.IsFalse(WikiRegexes.Stub.IsMatch(text), "stub");
            // ClassicAssert.IsTrue(text.Contains("{{وصلات قليلة|" + WikiRegexes.DateYearMonthParameter + @"}}"),"wikify");
...
[[link]]" + LongText, "Test", false, out noChange, ref summary);
            ClassicAssert.IsFalse(WikiRegexes.DeadEnd.IsMatch(text), "deadend");
            ClassicAssert.IsTrue(WikiRegexes.Orphan.IsMatch(text), "orphan");
            ClassicAssert.IsTrue(WikiRegexes.Wikify.IsMatch(text), "wikify");
            ClassicAssert.IsFalse(text.Contains("}}" + "\r\n\r\n" + "{{")); // no blank line between wikify & orphan
        ...
        public void AddArz()
        {
            ...
            string text = parser.Tagger(ShortText, "Test", false, out noChange, ref summary);
            // Stub, no existing stub tag. Needs all tags
            ClassicAssert.IsTrue(text.Contains("{{يتيمه|" + WikiRegexes.DateYearMonthParameter + @"}}"), "orphan");
            // ClassicAssert.IsFalse(text.Contains("{{ويكى|" + WikiRegexes.DateYearMonthParameter + @"}}"),"wikify");
            // ClassicAssert.IsTrue(WikiRegexes.DeadEnd.IsMatch(text));
            ClassicAssert.IsTrue(Tools.NestedTemplateRegex("تقاوى مش متصنفه").IsMatch(text), "Uncategorized stub");
            ClassicAssert.IsTrue(WikiRegexes.Stub.IsMatch(text), "stub tag added");

            text = parser.Tagger(ShortText + @"{{disambig}}", "Test", false, out noChange, ref summary);
            // Don't add stub/orphan to disambig pages. Still add wikify
            ClassicAssert.IsFalse(text.Contains("{{يتيمه|" + WikiRegexes.DateYearMonthParameter + @"}}"), "orphan not added");
            ClassicAssert.IsFalse(WikiRegexes.Stub.IsMatch(text), "stub tag not added");
            ClassicAssert.IsFalse(text.Contains("{{ويكى|" + WikiRegexes.DateYearMonthParameter + @"}}"), "wikify not added");

            text = parser.Tagger(ShortText + @"{{توضيح}}", "Test", false, out noChange, ref summary);
            // Don't add stub/orphan to disambig pages. Still add wikify
            ClassicAssert.IsFalse(text.Contains("{{يتيمه|" + WikiRegexes.DateYearMonthParameter + @"}}"), "orphan not added");
            ClassicAssert.IsFalse(WikiRegexes.Stub.IsMatch(text), "stub not added");
            ClassicAssert.IsFalse(text.Contains("{{ويكى|" + WikiRegexes.DateYearMonthParameter + @"}}"), "wikify not added");
        ...
        public void AddOrphan()
        {
            ...
            ClassicAssert.IsFalse(WikiRegexes.Orphan.IsMatch(text), "pages using {{wi}} not tagged as orphan");

            text = parser.Tagger(ShortText + @"{{List of lists}}", "Test", false, out noChange, ref summary);
            ClassicAssert.IsFalse(WikiRegexes.Orphan.IsMatch(text), "pages using {{List of lists}} not tagged as orphan");
            ...
            ClassicAssert.IsFalse(WikiRegexes.Orphan.IsMatch(text), "soft redirect not tagged as orphan");
            ...
            ClassicAssert.IsFalse(WikiRegexes.Orphan.IsMatch(text), "tags with subst");
        ...
        public void AddTagsWikia()
        {
            ...
            ClassicAssert.IsFalse(WikiRegexes.Orphan.IsMatch(text), "No orphan tag added on Wikia");
        ...
        public void AddLinklessRu()
        {
            ...
            ClassicAssert.IsFalse(WikiRegexes.Orphan.IsMatch(text));
            ClassicAssert.IsTrue(text.Contains(@"{{rq|wikify|style|linkless}}"));
            ...
            ClassicAssert.IsTrue(WikiRegexes.Orphan.IsMatch(text));
            ClassicAssert.IsFalse(text.Contains(@"|linkless}}"));
            ...
            ClassicAssert.IsTrue(WikiRegexes.Orphan.IsMatch(text), "page is orphan");
            ClassicAssert.IsFalse(text.Contains(@"{{rq|wikify|style|linkless}}"));
#endif
        }
...
| PLACE OF DEATH    = Earth
        ...
        public void RemoveDisambig()
        {
            ...
            ClassicAssert.IsTrue(WikiRegexes.DisambigsCleanup.IsMatch(text), "keeps disambig cleanup");
            ClassicAssert.IsFalse(WikiRegexes.DisambigsGeneral.IsMatch(text), "removes disambig");
...
{{orphan}}
            ...
            ClassicAssert.IsFalse(WikiRegexes.Orphan.IsMatch(text));
            ClassicAssert.IsFalse(WikiRegexes.MultipleIssues.IsMatch(text));
            Assert.That(text, Is.EqualTo("[[foo]]"));
        ...
        public void MultipleIssuesNewTags()
        {
            ...
            ClassicAssert.IsFalse(WikiRegexes.Wikify.IsMatch(Text), "added tags go in existing multipleIssues");
        }

        [Test]
        public void RemoveDeadEnd()
        {
            ClassicAssert.IsFalse(WikiRegexes.DeadEnd.IsMatch(parser.Tagger(@"foo {{deadend|date=May 2010}} [[a]] and [[b]] and [[b]]", "Test", false, out noChange, ref summary)));
            ClassicAssert.IsTrue(summary.Contains("removed deadend tag"));
...
{{stub}}"));
            ...
            ClassicAssert.IsFalse(WikiRegexes.DeadEnd.IsMatch(parser.Tagger(@"foo {{Multiple issues|COI = May 2010 |POV = June 2010 | dead end=May 2010}} [[a]] and [[b]] and [[b]]", "Test", false, out noChange, ref summary)));

            ClassicAssert.IsTrue(WikiRegexes.DeadEnd.IsMatch(parser.Tagger(@"foo==x== {{deadend|section|date=May 2010}} [[a]] and [[b]] and [[b]]", "Test", false, out noChange, ref summary)), "does not remove section tags");
            ClassicAssert.IsFalse(summary.Contains("removed deadend tag"));

            ClassicAssert.IsTrue(WikiRegexes.DeadEnd.IsMatch(parser.Tagger(@"foo==x== {{deadend|date=May 2010}} {{Proposed deletion/dated|[[a]] and [[b]] and [[b]]}}", "Test", false, out noChange, ref summary)),
                          @"does not remove {{dead end}} when links are within prod template");
            ClassicAssert.IsTrue(WikiRegexes.DeadEnd.IsMatch(parser.Tagger(@"foo==x== {{deadend|date=May 2010}} {{Proposed deletion endorsed|[[a]] and [[b]] and [[b]]}}", "Test", false, out noChange, ref summary)),
        ...
        public void RemoveDeadEndAr()
        {
            ...
            ClassicAssert.IsFalse(WikiRegexes.DeadEnd.IsMatch(parser.Tagger(@"foo {{نهاية مسدودة|تاريخ=ديسمبر 2012}} [[a]] and [[b]] and [[b]]", "Test", false, out noChange, ref summary)));
            ClassicAssert.IsTrue(summary.Contains("نهاية مسدودة"));

            ClassicAssert.IsFalse(WikiRegexes.DeadEnd.IsMatch(parser.Tagger(@"foo {{dead end|date=December 2012}} [[a]] and [[b]] and [[b]]", "Test", false, out noChange, ref summary)));
            ClassicAssert.IsTrue(summary.Contains("نهاية مسدودة"));
        ...
        public void UpdateFactTag()
        {
            ...
            ClassicAssert.IsTrue(text.Contains("{{fact|date={{subst:CURRENTMONTHNAME}}"));
            ClassicAssert.IsFalse(text.Contains("{{fact}}"));
        ...
        public void UpdateCitationNeededTag()
        {
            ...
            ClassicAssert.IsTrue(text.Contains(@"{{citation needed|date={{subst:CURRENTMONTHNAME}} {{subst:CURRENTYEAR}}}}"));
            ...
            ClassicAssert.IsTrue(text.Contains(@"{{citation needed|reason=something|date={{subst:CURRENTMONTHNAME}} {{subst:CURRENTYEAR}}}}"));
            ...
            ClassicAssert.IsTrue(text.Contains(correctcn));

            const string commentText = "<!--{{citation needed}}-->";

            text = parser.Tagger(commentText, "Test", false, out noChange, ref summary);
            ClassicAssert.IsTrue(text.Contains(commentText), "tag not dated when commented out");

            text = parser.Tagger(@"{{Citation needed|Date=May 2009}}", "Test", false, out noChange, ref summary);
            ClassicAssert.IsTrue(text.Contains(@"{{Citation needed|date=May 2009}}"), "Date -> date");

            text = parser.Tagger(@"{{Citation needed|date=May 2009}}", "Test", false, out noChange, ref summary);
            ClassicAssert.IsTrue(text.Contains(@"{{Citation needed|date=May 2009}}"), "if tag already dated, no change");
        }
        ...
        public void UpdateWikifyTag()
        {
            ...
            ClassicAssert.IsTrue(text.Contains(correct));
            ClassicAssert.IsFalse(text.Contains("{{wikify}}"));

            text = parser.Tagger("{{template:wikify  }}", "Test", false, out noChange, ref summary);
            ClassicAssert.IsTrue(text.Contains(@"|date={{subst:CURRENTMONTHNAME}} {{subst:CURRENTYEAR}}}}"));

            text = parser.Tagger("{{wikify section}}", "Test", false, out noChange, ref summary);
            ClassicAssert.IsTrue(text.Contains(@"{{wikify section|date={{subst:CURRENTMONTHNAME}} {{subst:CURRENTYEAR}}}}"));
        ...
        {
            ClassicAssert.IsTrue(Parsers.TaggerPermitted("A", "Wikipedia:AutoWikiBrowser/Sandbox"));
            ClassicAssert.IsFalse(Parsers.TaggerPermitted("A", "Wikipedia:ABC"));
            ClassicAssert.IsFalse(Parsers.TaggerPermitted("A {{wi}}", "ABC"));
            ClassicAssert.IsFalse(Parsers.TaggerPermitted("A", "Main Page"));
...
Chris.";
        ...
        public void TagRefsIbid()
        {
            ...
            ClassicAssert.IsTrue(returned.Contains(@"{{Ibid|date="));
            ClassicAssert.IsTrue(summary.Contains("Ibid"));
            ...
            ClassicAssert.IsFalse(returned.Contains(@"{{Ibid|date="));
            ClassicAssert.IsFalse(summary.Contains("Ibid"));

            returned = parser.Tagger(@"now<Ref name=""again"">IBID</ref> was", "test", false, ref summary);
            ClassicAssert.IsTrue(returned.Contains(@"{{Ibid|date="), "Case-insensitive check for ibid");
        }
...
";
            ...
            ClassicAssert.IsTrue(returned.Contains(@"==Foo1==
{{Empty section|date={{subst:CURRENTMONTHNAME}} {{subst:CURRENTYEAR}}}}

==Foo2==
"), "Commented out text not changed");
            ClassicAssert.IsTrue(summary.Contains("Empty section (1)"));
...
"), "Extra whitespace handled");
            ClassicAssert.IsTrue(summary.Contains("Empty section (3)"));
...
";
            returned = parser.Tagger(twoTwos, "test", false, ref summary);
            ClassicAssert.IsFalse(returned.Contains(@"{{Empty section|date={{subst:CURRENTMONTHNAME}} {{subst:CURRENTYEAR}}}}"), "Section not empty");
...
";
            returned = parser.Tagger(twoTwos, "test", false, ref summary);
            ClassicAssert.IsFalse(returned.Contains(@"{{Empty section|date={{subst:CURRENTMONTHNAME}} {{subst:CURRENTYEAR}}}}"), "Empty level 3 sections not tagged");
