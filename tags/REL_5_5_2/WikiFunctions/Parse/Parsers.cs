...
namespace WikiFunctions.Parse
{
    ...
    public class Parsers
    {
        ...
        private static readonly Regex PunctuationAfterSfn = new Regex(@"(?<sfn>" + Tools.NestedTemplateRegex(new[] {"Sfn", "Shortened footnote", "Shortened footnote template", "efn"}).ToString() + @")(?<punc>[,\.;:])");
        private static readonly Regex SfnAfterDupePunctuation = new Regex(@"([^,\.:;])" + RefsPunctuation + @"\2 *(?<sfn>" + Tools.NestedTemplateRegex(new[] {"Sfn", "Shortened footnote", "Shortened footnote template", "efn"}).ToString() + @")");
        ...
        private static readonly Regex citeWebParameters = new Regex(@"\b(accessdate|agency|archivedate|archiveurl|arxiv|asin|at|author\d?|authorlink\d?|authors|author-mask|author-name-separator|author-separator|bibcode|coauthors?|contribution|date|deadurl|display-authors|doi|doibroken|editor|editor\d*-first|editor\d*-last|editor\d*-link|first\d*|format|id|isbn|issn|jfm|jstor|language|last\d*|lastauthoramp|laydate|laysource|layrul|lccn|location|month|mr|oclc|ol|origyear|others|osti|pages?|pmc|pmid|postscript|publisher|quote|ref|registration|rfc|separator|series|subscription|ssrn|title|trans_title|type|url|via|website|work|year|zbl)\b", RegexOptions.Compiled);
        ...
        private static readonly Regex ProposedDeletionDatedEndorsed = Tools.NestedTemplateRegex( new [] {"Proposed deletion/dated", "Proposed deletion endorsed" });
